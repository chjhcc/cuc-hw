...
class key_press:
    ...
    def percentage(self):
        global storage, vartext, result, symbol
        if vartext.get() == '' or vartext.get() == '0':  # 如果vartext中没有值，则什么也不做
            pass
        else:
    ...
    def operation(self):
        ...
        if vartext.get() == '':		#如果vartext中没有值，则什么也不做
            ...
            elif  self.key in ('+', '-', '*', '/', 'sin','cos','='):				#判断按键值
                if symbol is None:  #如果没有输入符号
                    result = get1						#获取get1的值

                else:  #如果symbol变量不是None
...
def layout(window):
    ...
    buttonCE=tkinter.Button(window,text='|',width=5,command=key_press('|').abs)
    ...
    buttons=tkinter.Button(window,text='sin',width=5, command=key_press('sin').operation)
    buttoncos=tkinter.Button(window,text='cos',width=5, command=key_press('cos').operation)
    buttonJ.grid(row=2,column=0)
    buttonCE.grid(row=2,column=1)
    buttonC.grid(row=2,column=2)
    button12.grid(row=2,column=3)
    buttonD.grid(row=2,column=4)
    buttons.grid(row=2,column=5)
    ...
    buttonf= tkinter.Button(window, text=' % ',width=5,command=key_press('/').percentage)
